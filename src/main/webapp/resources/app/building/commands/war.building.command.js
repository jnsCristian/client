	...
	atlantis.eventManger.addListener('get.war.building.success', function() {
		...
		if (troops.troopsDto.length) {
			for (var i = 0; i < troops.troopsDto.length; i++) {
				...
				troopMarkup += '<br><div class="troop'+ troops.troopsDto[i].id + '"><td><img src="resources/img/troopsBuilding/'+ troops.troopsDto[i].id + '.png" class="troop">'+ String(troops.troopsDto[i].name)+ '  </td><td><input type="number" min="0" class="border" data-troop-id="'+ troops.troopsDto[i].id + '"><button class="button troops-train " data-troop-id="'+ troops.troopsDto[i].id + '">Train</button></td></div>   Cost:'
			...
			troopMarkup += '<br><div class="troop'+ troops.troopsDto.id + '"><td><img src="resources/img/troopsBuilding/'+ troops.troopsDto.id + '.png"class="troop">  '+ String(troops.troopsDto.name)+ '  </td><td><input type="number" min="0" class="border" data-troop-id="'+ troops.troopsDto.id + '"><button class="button troops-train " data-troop-id="'+ troops.troopsDto.id + '">Train</button></td></div>  Cost:'
			for (var j = 0; j < cost.length; j++) {
				troopMarkup += '<td> '+ String(cost[j].cost)+ '  </td><td> '+ String(cost[j].resourceName)+ '  </td>'
			}
			troopMarkup += '<br><td> Skills: '+ String(skills.armor)+ '  armor </td><td>'+ String(skills.speed)+ ' speed  </td>'+ '  </td><td> '+ String(skills.demage)+ ' demage </td>'
